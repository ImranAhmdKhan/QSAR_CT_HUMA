...
for (i in 1:100){
  ...
  # mdl <- lm(formula = "Response~1 + ndonr + Shet + GATSe1 + MATSm4 + MATSm5 + MATSm6", 
            #data = trainingset) # using the same descriptors to build the model
    mdl <- lm(formula = "Response~1 + Lowest.Frequency..cm.1. + Balaban.type.index.from.van.der.waals.weighted.distance.matrix + Wiener.type.index.from.van.der.waals.weighted.distance.matrix + AlogP + HOMO.1", 
              data = trainingset) # using the same descriptors to build the model
  
  predict <- predict(mdl,newdata = testset)
  fitted <- mdl$fitted.values
  a <- eval_results(testset$Response,predict,testset)
  b <- eval_results(trainingset$Response,fitted,trainingset)
